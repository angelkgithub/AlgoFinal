...
class ClassScheduler:
        ...
        if hours <= 2 and not should_split:
            ...
            for day in self.days:
                for start_time in self.time_slots[:-4]:
                    ...
                    if end_time and self._is_valid_end_time(end_time):
                        patterns.append([{
                            'day': day,
            ...
            slots_needed = max(3, int(session_hours * 2) + 1)
    ...
    def _is_valid_end_time(self, end_time_str):
        """Check if end time is valid (before 9 PM and within reasonable bounds)"""
        try:
            end_time = datetime.strptime(end_time_str, "%H:%M").time()
            max_time = datetime.strptime("21:00", "%H:%M").time()
            return end_time <= max_time
        except:
            return False
    ...
    def _add_hours_to_time(self, time_str, hours):
        """Add hours to a time string and round to nearest 30-minute slot"""
        try:
            time_obj = datetime.strptime(time_str, "%H:%M").time()
            datetime_obj = datetime.combine(datetime.today(), time_obj)
            new_datetime = datetime_obj + timedelta(hours=hours)
            
            # Round to nearest 30-minute slot for consistency
            ...
            if minutes % 30 != 0:
                ...
                if minutes % 30 < 15:
                    ...
                    if minutes == 60:
                        ...
                        minutes = 0
                new_datetime = new_datetime.replace(minute=minutes, second=0, microsecond=0)
